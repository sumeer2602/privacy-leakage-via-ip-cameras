...
def analyze_pcap(interface=None, file_path=None):
    ...
    if file_path:
        ...
        print(f"Analyzing live traffic on {interface} between {camera_ip} and {server_ip}...")
        capture = pyshark.LiveCapture(interface=interface, display_filter=f'ip.addr == {camera_ip} and ip.addr == {server_ip}')
...
def main():
    ...
    parser.add_argument("-f", "--file", help="Path to the pcap file. Leave empty to analyze live traffic.")
    parser.add_argument("-i", "--interface", help="Specify the interface for live capture.")
    args = parser.parse_args()

    if not args.interface and not args.file:
        # If no interface is specified, list available interfaces and prompt user
        interfaces = list_interfaces()
        interface_idx = int(input("Select the interface number to use: ")) - 1
        args.interface = interfaces[interface_idx]

    analyze_pcap(interface=args.interface, file_path=args.file)
